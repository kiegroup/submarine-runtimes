...
public interface KogitoBuildContext {
    ...
    static String generateRESTConfigurationKey(String generatorType) {
    ...
    /**
     * Method to check if REST for a specific resource is available and enabled.
     * This is platform/classpath specific (e.g. Quarkus) but it can also be explicitly disabled i.e.
     * kogito.generate.rest.decisions = false
     */
    default boolean hasREST(Generator generator) {
        return getRestAnnotator() != null &&
                "true".equalsIgnoreCase(getApplicationProperty(generateRESTConfigurationKey(generator.name())).orElse("true"));
    }
